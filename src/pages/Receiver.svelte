...
function startRecording() {
	...
	captureAudio(recordingContext, {deviceId: localStorage.audioInputDeviceId, chunkSize:4096}, (audioBuffer, packetNum) => {
		...
		if(localStorage.fillSine == "true") {
			fillSine(audioBuffer)
        }

		sentPacketNum  = packetNum
		const delayMs = 300
		const packet = new Packet([Messages.AUDIO_DATA, PUB_CHANNEL, packetNum, delayMs], audioBuffer)
...
function subscribe() {
    const packet = new Packet([Messages.SUBSCRIBE, SUB_CHANNEL, 1])
	sock.emit(packet)
	subscribed = true
}

function unsubscribe() {
    const packet = new Packet([Messages.SUBSCRIBE, SUB_CHANNEL, 0])
	sock.emit(packet)
	subscribed = false
}
